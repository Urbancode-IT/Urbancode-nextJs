import { fetchCrmCourseNames, resolveCrmCourseName } from "./resolveCrmCourse";
...
export async function sendExternalEnrollment(data) {
  ...
  const crmCourseNames = await fetchCrmCourseNames();
  const course = resolveCrmCourseName(rawCourse, crmCourseNames);
  const requirements = [
    data.requirements || "",
    rawCourse && rawCourse !== course ? `Website course: ${rawCourse}` : "",
  ]
    .filter(Boolean)
    .join(" | ");
