...
//-----------Submit IELTS/PTE Evaluation Form-----------------
export const submitIeltsEvaluationForm = async (formData) => {
    try {
      const response = await fetchWithTimeout(`/api/send-email/ielts-evaluation`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
      ...
      console.error("API Error:", error);
      const msg = error.name === 'AbortError' ? 'Request timed out. Please try again.' : (error.message || "Failed to send evaluation.");
      return { success: false, message: msg };
    }
};
...
//---------------English Language Intake Form------------------
export const submitEnglishIntakeForm = async (formData) => {
    try {
      const response = await fetchWithTimeout(`/api/send-email/english-intake`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
      ...
      console.error("API Error:", error);
      const msg = error.name === 'AbortError' ? 'Request timed out. Please try again.' : (error.message || "Failed to send intake form.");
      return { success: false, message: msg };
    }
};

//---------------Enroll Now Form------------------
export const submitEnrollNowForm = async (formData) => {
    try {
      const response = await fetchWithTimeout(`/api/send-email/english-proficiency`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
      ...
      return { success: true, message: "Enrollment form submitted successfully!" };
    } catch (error) {
      console.error("API Error:", error);
      const msg = error.name === 'AbortError' ? 'Request timed out. Please try again.' : (error.message || "Failed to send enrollment form.");
      return { success: false, message: msg };
    }
};